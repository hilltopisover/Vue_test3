  ...
  <div class="main-box">
    ...
    >
      <form>
        ...
        <!-- <router-link to="/a3/student" v-if="isLogin">
          <div class="primary-btn" @click="login">立即登录</div>
        </router-link>
        <router-link to="/a3/student" v-else>
          <div class="primary-btn" @click.prevent="login">立即登录</div>
        </router-link> -->
      </form>
    </div>
...
export default {
  ...
  methods: {
    login() {
      axios.post('http://127.0.0.1:5000/api/login', this.loginForm)
        .then(response => {
          // 处理登录成功的情况
          this.isLogin = true;
          console.log("登录成功")
    ...
    register() {
      // 在这里添加注册逻辑
      // 例如，使用 axios 发送注册请求
      axios.post('http://127.0.0.1:5000/api/register', this.registerForm)
        .then(response => {
          // 处理注册成功的情况
          this.isLogin = true;
          console.log("注册成功")
...
import { serverToken, serverUsername, resetRoutes} from '../router/a5router';
...
const login = () => {
  axios.post('http://127.0.0.1:5000/api/login', loginForm.value)
    .then(response => {
      // 处理登录成功的情况
      isLogin.value = true
      console.log("登录成功")
...
const register = () => {
  ...
  axios.post('http://127.0.0.1:5000/api/register', registerForm.value)
    .then(response => {
      // 处理注册成功的情况
      isLogin.value = true
      console.log(registerForm.value)
...
function getUsername(token:string){
  ...
  const s = token.split(".")
  return JSON.parse(atob(s[1])).username
}
</script>
